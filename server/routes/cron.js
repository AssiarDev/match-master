...
router.post('/cron/update-db/players', async (req, res) => {
    try {
        ...
        res.status(500).send('Une erreur est survenue lors de la mise à jour des joueurs.')
    }
});
...
router.post('/cron/update-db/trainers', async (req, res) => {
    try {
        ...
        res.status(500).send('Une erreur est survenue lors de la mise à jour des entraineurs.')
    }
})
