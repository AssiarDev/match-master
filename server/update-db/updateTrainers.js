    ...
    await Promise.all(
      trainers.map(async (trainer) => {
        ...
        if (clubExists.rowCount === 0) {
          console.warn(
            `Club ID ${trainer.id_club} n'existe pas, entraîneur ignoré`
          );
          return;
        }

        const existingTrainer = await pool.query(
          'SELECT id_club, contract_start, contract_end FROM "Trainer" WHERE id = $1',
          [trainer.id]
        );

        if (existingTrainer.rowCount === 0) {
          console.log(
            `Entraîneur ${trainer.name} non trouvé, aucune mise à jour.`
          );
          return;
        }
        ...
        if (
          ...
          await pool.query(
            `
                        ...
                        WHERE id = $4;
                    `,
            [trainer.id_club, newStart, newEnd, trainer.id]
          );
