...
describe("StandingService", () => {
  ...
  it("retourne une erreur si getLeagueCurrentSeason échoue", async () => {
    ...
    expect(result).toEqual({
      success: false,
      message:
        "Impossible de récupérer le classement Error: Erreur API",
    });
  });
  ...
  it("retourne une erreur si fetchStandingBySeason échoue", async () => {
    ...
    expect(result).toEqual({
      success: false,
      message:
        "Impossible de récupérer le classement Error: DB error",
    });
  });
  ...
  it("retourne une erreur si teamsByIds échoue", async () => {
    ...
    standingRepoMock.fetchStandingBySeason?.mockResolvedValue({
      data: [
        { participant_id: 10, team_id: 10, details: [] } as any,
      ],
    });
    ...
    expect(result).toEqual({
      success: false,
      message:
        "Impossible de récupérer le classement Error: Teams error",
    });
  });
});